package genome_browser::genome_browserServer;
...
sub call_method {
    ...
    my $ctx = genome_browser::genome_browserServerContext->new($self->{loggers}->{userlog},
...
{
    # Service genome_browser requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for genome_browser but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = genome_browser::genome_browserServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service genome_browser.");
    }
...
package genome_browser::genome_browserServerContext;

use strict;

=head1 NAME

genome_browser::genome_browserServerContext
...
package genome_browser::genome_browserServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = genome_browser::genome_browserServer->new(
