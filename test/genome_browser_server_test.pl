...
my $config = new Config::Simple($config_file)->get_block('genome_browser');
...
$genome_browser::genome_browserServer::CallContext = $ctx;
...
sub get_ws_name {
    if (!defined($ws_name)) {
        ...
        $ws_name = 'test_genome_browser_' . $suffix;
...

{
    ...
    sub provenance {
        my($self) = @_;
        return [{'service' => 'genome_browser', 'method' => 'please_never_use_it_in_production', 'method_params' => []}];
    }
    sub authenticated {
        return 1;
    }
